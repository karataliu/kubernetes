...
### Changes for runtime
Three runtime binaries are involved: `kubelet`, `apiserver`, `controller-manager`. Currently all of them have dependent logic on cloud provider, and thus accepts a parameter '--cloud-provider'.
...
`controller-manager`    : Disk attach/provision via cloud provider (with [EnableControllerAttachDetach](https://github.com/kubernetes/kubernetes/pull/26351) on)
...
#### Dependencies
- azure_dd
1. attacher.go      : Calls get nodename to instance id mapping
2. attacher.go      : Calls Azure provider to do attach/deattach data disk
3. azure_common.go  : Calls MakeCRC32 util func
...
2. Kubelet still depends on the volume plugin which relies on Cloud Provider
    ...
    Pending issue, should come out with solution together with#1
