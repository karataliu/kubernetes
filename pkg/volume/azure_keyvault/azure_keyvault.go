...
type kvApi struct {
	environment autorest.Environment
...
// Vault API
func createKVCApi(tenantId, clientId, clientSecret string, env autorest.Environment) (*kvApi, error) {
	kvapi := &kvApi{environment: env}

	oauthConfig, err := kvapi.environment.OAuthConfigForTenant(tenantId)
	if err != nil {
		return nil, err
	}

	servicePrincipalToken, err := autorest.NewServicePrincipalToken(
		*oauthConfig,
		clientId,
		clientSecret,
	...
	kvapi.kvClient.Authorizer = servicePrincipalToken
	return kvapi, nil
}
